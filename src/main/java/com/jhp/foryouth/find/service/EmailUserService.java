...
public class EmailUserService {
    ...
    public void sendEmailAboutUserId(String userId, String userEmail) throws MessagingException {
    ...
    public void sendEmailAboutCode(String code, String userEmail) throws MessagingException {
        LocalDateTime now = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-mm-dd HH:mm");
        String formattedNow = now.format(formatter);

        String title = "ForYouth : 비밀번호 찾기 관련 인증코드";
        ...
        String content =
                "<div style='font-family: Arial, sans-serif; padding: 20px; background-color: #f4f4f4;'>"
                        + "  <div style='max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);'>"
                        + "    <h2 style='color: #333;'>안녕하세요, 청년들을 위한 플랫폼 ForYouth입니다:) 📖</h2>"
                        + "    <p style='font-size: 16px; color: #555;'>회원님은 <strong>" + formattedNow + "</strong>에 인증코드를 요청하셨습니다.</p>"
                        + "    <p style='font-size: 16px; color: #555;'>인증코드는 <strong>" + code + "</strong> 입니다.</p>"
                        + "    <p style='font-size: 16px; color: #555;'><strong>ForYouth</strong>를 이용하시며 항상 유익한 시간 되시길 바랍니다. 감사합니다:)</p>"
                        + "    <p style='font-size: 16px; color: #555; margin-top: 50px;'><strong>ForYouth 드림</strong></p>"
                        + "  </div>"
                        + "</div>";
    ...
    public void sendEmail(String to, String title, String content) throws MessagingException {
        ...
        log.info("인증코드 이메일을 {}님에게 전송했습니다!", to);
